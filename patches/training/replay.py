"""Replay buffer primitives for dict-observation DQN training."""
...
class ReplayBuffer:
    ...
    def add(self, transition: Transition) -> None:
        stored = Transition(
            obs=_copy_observation(transition.obs),
            action=int(transition.action),
            reward=float(transition.reward),
            next_obs=_copy_observation(transition.next_obs),
    ...
    def sample(self, batch_size: int) -> TransitionBatch:
        ...
        return TransitionBatch(
            obs=_stack_observations([transition.obs for transition in transitions]),
            action=np.asarray(
            ...
            next_obs=_stack_observations(
                [transition.next_obs for transition in transitions]
            ),
...
def _copy_observation(obs: Mapping[str, np.ndarray]) -> Observation:
    return {
...
def _stack_observations(observations: list[Mapping[str, np.ndarray]]) -> Observation:
    if not observations:
        raise ValueError("cannot stack an empty observation list")
    keys = tuple(observations[0])
    return {
        key: np.stack([np.asarray(obs[key], dtype=np.float32) for obs in observations])
        for key in keys
    }
