"""Difficulty schedules and puzzle sampling helpers."""
...
from zip.simulation import Puzzle, generate_puzzle
from zip.simulation.config import DIFFICULTIES, get_difficulty_config

DIFFICULTY_ORDER = ("super_easy", "easy", "medium", "hard", "expert")

DEFAULT_EVALUATION_SEEDS: dict[str, range] = {
    "super_easy": range(1000, 1100),
    "easy": range(2000, 2100),
    "medium": range(3000, 3100),
    "hard": range(4000, 4100),
    "expert": range(5000, 5100),
}
...
def build_default_curriculum(target_difficulty: str = "medium") -> tuple[CurriculumStage, ...]:
    """Build the default easier-to-harder Zip curriculum.
    ...
    for index in range(1, target_index + 1):
        ...
        stages.append(
            CurriculumStage(
                current,
                (current,),
                min_success_rate=None if index == target_index else 0.95,
                eval_difficulty=current,
            )
        )
    return tuple(stages)
...
class AsyncPuzzleBuffer:
    ...
    def sample_puzzle(self) -> Puzzle:
        ...
        while True:
            ...
            if self._pending:
                futures = [future for version, future in self._pending if version == self.version]
                if futures:
...
def curriculum_from_name(
    ...
    *,
    ...
    if normalized not in {"default", "zip", "auto"}:
        raise ValueError("curriculum must be one of: none, default, zip, auto")
