"""Real-time Pygame visual training for Zip DQN agents."""
...
from zip.simulation.config import DIFFICULTIES, UIConfig
from zip.simulation.ui import BoardLayout, Button, PathMotion, ZipGameUI
...
from .env import ACTION_TO_DIRECTION, ZipEnv
...
class VisualEnvState:
    ...
    path_motion: PathMotion | None = None
    episode_index: int = 1
...
class VisualTrainingUI(ZipGameUI):
    """Train a DQN agent while rendering each environment step in Pygame."""

    def __init__(
        self,
        env: ZipEnv | Sequence[ZipEnv],
        agent: ZipAgent,
        *,
        checkpoint_dir: str | Path = "checkpoints/zip_visual",
        ...
        envs = [env] if isinstance(env, ZipEnv) else list(env)
        if not envs:
        ...
        for index, training_env in enumerate(self.envs):
            ...
            self.slots.append(
                VisualEnvState(
                    ...
                    obs=obs,
                    info=info,
                    visual_path=list(training_env.state.path),
                )
            )
        ...
        super().__init__(
            ...
            save_path=Path("zip_visual_training_puzzle.json"),
            config=config,
        )
    ...
    def _sync_primary_aliases(self) -> None:
        ...
        self.obs = slot.obs
        self.info = slot.info
        self.visual_path = list(slot.visual_path)
        self.path_motion = slot.path_motion
    ...
    def _fit_layout_to_window_limits(self) -> None:
        ...
        fitted_cell_size = max(18, int(fitted_cell_size))
        ...
        for index, slot in enumerate(self.slots):
            ...
            board_width = puzzle.cols * cell_size
            board_height = puzzle.rows * cell_size
            origin_x = (
                self.config.margin
    ...
    def _train_one_step(self, slot_index: int = 0) -> None:
        ...
        slot.obs = next_obs
        slot.info = next_info
        self._sync_from_env(slot_index, old_path=old_path, animate=True)
    ...
    def _new_training_episode(self, slot_index: int = 0) -> None:
        ...
        self.training_accumulator = 0.0
        self._sync_from_env(slot_index, old_path=None, animate=False)
    ...
    def _sync_from_env(
        ...
        slot_index: int = 0,
        *,
        old_path: list[tuple[int, int]] | None,
        animate: bool,
    ) -> None:
        slot = self.slots[slot_index]
        ...
        if old_path is None:
            slot.visual_path = list(state.path)
            slot.path_motion = None
        else:
            self._start_slot_path_motion(slot, old_path, list(state.path), animate=animate)
        if layout_changed:
            self._fit_layout_to_window_limits()
            self._rebuild_buttons()
        self._sync_primary_aliases()

    def _start_slot_path_motion(
        self,
        slot: VisualEnvState,
        ...
        *,
        ...
        if animate and is_single_step and shares_prefix and old_path[-1] != new_path[-1]:
            slot.path_motion = PathMotion(old_path, new_path)
        else:
            slot.visual_path = list(new_path)
            slot.path_motion = None

    def _update_path_motion(self, dt: float) -> None:
        for slot in self.slots:
            if slot.path_motion is None:
                continue
            slot.path_motion.elapsed += dt
            if slot.path_motion.elapsed >= self.config.path_animation_seconds:
                slot.visual_path = list(slot.path_motion.to_path)
                slot.path_motion = None
        self._sync_primary_aliases()
    ...
    def _draw(self, screen: object, font: object, small_font: object, pygame: object) -> None:
        ...
        self._draw_training_boards(screen, small_font, pygame)
        self._draw_panel(screen, font, small_font, pygame)

    def _draw_training_boards(self, screen: object, small_font: object, pygame: object) -> None:
        original = (
            self.puzzle,
            self.state,
            self.visual_path,
        ...
        for index, (slot, layout) in enumerate(zip(self.slots, self.board_layouts)):
            ...
            board_font = pygame.font.SysFont(
                "arial",
                max(14, int(layout.cell_size * 0.38)),
                True,
            )
            self._draw_board_content(screen, board_font, pygame, layout)

        self.puzzle, self.state, self.visual_path, self.path_motion, self.layout = original
        self._sync_primary_aliases()
    ...
    def _draw_panel(self, screen: object, font: object, small_font: object, pygame: object) -> None:
        ...
        lines = [
            ...
            f"visited: {len(primary_state.visited)}/{primary_puzzle.total_cells}",
            "",
        ...
        for line in lines:
            if not line:
                y += 10
                continue
            surface = small_font.render(line, True, (30, 30, 28))
    ...
    def _training_panel_lines(self) -> list[str]:
        ...
        action_text = "-"
        if primary.last_action is not None:
            action_text = ACTION_TO_DIRECTION[primary.last_action].name.lower()
        mean_reward = (
...
def main(argv: list[str] | None = None) -> None:
    ...
    agent = ZipAgent(envs[0], config, checkpoint_path=args.checkpoint_path)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints/zip_visual")
    ...
    parser.add_argument("--steps-per-second", type=float, default=6.0)
