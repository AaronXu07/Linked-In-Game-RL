"""DQN agent wrapper for Zip environments."""
...
class ZipAgent:
    def __init__(
        ...
        if config.algorithm != "dqn":
            raise ValueError("only dqn is implemented")
        ...
        self.online_net = ZipDQN(observation_shape, action_count).to(self.device)
        self.target_net = ZipDQN(observation_shape, action_count).to(self.device)
    ...
    def predict(
        ...
        *,
        ...
        with torch.no_grad():
            q_values = self.online_net(obs_tensor).squeeze(0).detach().cpu().numpy()
    ...
    def evaluate(
        ...
        for episode in range(episodes):
            ...
            while True:
                ...
                if terminated or truncated:
                    ...
                    visited_fractions.append(
                        info["visited_count"] / max(1, info["total_cells"])
                    )
                    break
        ...
        return {
            ...
            "mean_visited_fraction": float(np.mean(visited_fractions)),
        }
        ...
        torch.save(
            {
                ...
                "observation_shape": tuple(self.env.observation_space.shape),
